...
async def criar_usuario(usuario: UsuarioBase):
    ...
    db = SessionLocal()
     try:        
        novo_usuario = Usuario(nome=nome_padronizadoemail=email_padronizado)        
        db.add(novo_usuario)        
        db.commit()        
        ...
        return {            
            ...
            "detalhes": {                
                "nome_original": usuario.nome,                "nome_padronizado": nome_padronizado,                "email_gerado": email_padronizado            
                }        
            }    
        except Exception as e:       
             db.rollback()        
             raise HTTPException(status_code=400, detail=str(e))    finally:        
             db.close()
    @app.get("/usuarios/")
